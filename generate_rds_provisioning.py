#!/usr/bin/env python3
"""Génère rds_provisioning.py directement dans son état final des sections 1 à 10
...
Variables d'environnement requises (les mêmes que celles du README, section 1) :
  VPC_ID, PRIVATE_SUBNET_1, PRIVATE_SUBNET_2, USER_ID, ALLOWED_CIDR, MASTER_PASSWORD
...
def main() -> None:
    ...
    parser.add_argument("--steps", type=int, default=10, choices=range(1, 11), help="générer jusqu'à cette section incluse (1-10, défaut 10)")
    parser.add_argument("--output-dir", default=".", help="répertoire où écrire rds_provisioning.py (défaut: répertoire courant)")
    args = parser.parse_args()

    required = ["VPC_ID", "PRIVATE_SUBNET_1", "PRIVATE_SUBNET_2", "USER_ID", "ALLOWED_CIDR", "MASTER_PASSWORD"]
